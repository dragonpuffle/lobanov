...
class TranscribeAudio[sessionT]:
    ...
    async def execute(self, session: sessionT, session_id: UUID, language: str = "ru") -> Transcript:
        ...
        if documentation_session.status != DocumentationSessionStatus.AUDIO_UPLOADED:
            error_message = f"Session must be in AUDIO_UPLOADED state, current state: {documentation_session.status}"
